...
- (instancetype)initWithDataArray:(NSArray *)dataArray
    ...
    if (self = [super initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT)]) {
        ...
        // from top
        if (direction == kLrdOutputViewDirectionTop)
        {
            self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, origin.y - 215, -width, height * _dataArray.count) style:UITableViewStylePlain];
        }
        // from down
        else
        {
            self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x, origin.y + 2, -width, height * _dataArray.count) style:UITableViewStylePlain];
        }
        
        _tableView.separatorColor = OMSeparatorLineColor;
        _tableView.backgroundColor = WhiteBackGroudColor;
        _tableView.bounces = NO;
        _tableView.layer.borderColor = OMSeparatorLineColor.CGColor;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.direction == kLrdOutputViewDirectionDown) {
        CGFloat startX = self.origin.x - 20;
        CGFloat startY = self.origin.y + 2;
    ...
    [WhiteBackGroudColor setFill]; //设置填充色

    [OMSeparatorLineColor setStroke];
